    ...
    base.initEvents = function () {
      base.$el.bind('mousedown touchstart touchmove touchend mousemove click', function (event) {
        ...
        if ((event.type === 'mousedown' && event.which === 1) || event.type === 'touchstart') {
		  AppCongif.releaseFrame = AppCongif.currentFrame; // Added to reset the "releaseFrame" to the position where the spinner has left; otherwise the board would first return to initial position
	...
	base.trackPointer = function (event) { 
		if (AppCongif.ready && AppCongif.dragging) { 
			if ( AppCongif.dragStart ) {
				...
				AppCongif.endFrame = AppCongif.releaseFrame + Math.ceil((AppCongif.totalFrames - 1) * AppCongif.speedMultiplier * (AppCongif.pointerDistance / base.$el.width()) * 0.15); // (0.15 because speedMultiplier doesn't seem to work)
				if( AppCongif.disableWrap ) {
